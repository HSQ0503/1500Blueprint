...
export function SetLibrary({ owned, shared }: { owned: FlashcardSet[]; shared: FlashcardSet[] }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-extrabold tracking-tight text-navy">Flashcards</h1>
          <p className="mt-1 text-sm text-navy/55">
            Study the sets your tutor shares, and build your own.
          </p>
        </div>
        <Link href="/flashcards/new" className={accentBtn}>
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        <div className="flex items-center rounded-card border border-navy/15 bg-white p-0.5">
      ...
      {shared.length > 0 ? (
        <section className="mt-7 rounded-card border border-gold-600/25 bg-gold/[0.05] p-4 sm:p-5">
          ...
          {sharedF.length === 0 ? (
            ...
            <SetGrid sets={sharedF} view={view} hrefBase="/flashcards" featured />
          )}
        </section>
      ) : null}
      ...
      <Section title="Your sets" count={owned.length}>
        {owned.length === 0 ? (
          ...
          <SetGrid sets={ownedF} view={view} hrefBase="/flashcards" />
        )}
      </Section>
    </div>
  );
...
function SetGrid({
  ...
  if (view === "list") {
    return (
      <div className="stagger flex flex-col gap-2">
        {sets.map((s) => (
          <SetCard key={s.id} set={s} href={`${hrefBase}/${s.id}`} layout="row" featured={featured} />
        ))}
      </div>
    );
  }
  return (
    <div className="stagger grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {sets.map((s) => (
        <SetCard key={s.id} set={s} href={`${hrefBase}/${s.id}`} featured={featured} />
      ))}
    </div>
  );
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center rounded-card border border-dashed border-navy/20 bg-white px-6 py-12 text-center">
      ...
      <Link href="/flashcards/new" className={`${accentBtn} mt-5`}>
        <PlusIcon className="h-4 w-4" />
