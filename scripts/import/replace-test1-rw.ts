/**
 * Replace only Practice Test 1's Reading/Writing content from a structured DOCX.
 * Existing module, question, and choice IDs are preserved; Math rows are never written.
 *
 *   npx tsx scripts/import/replace-test1-rw.ts "<rw-only.docx>" [--dry-run] [--cache=<path>]
 */
...
if (!docxPath) {
  console.error('Usage: tsx scripts/import/replace-test1-rw.ts "<rw-only.docx>" [--dry-run] [--cache=<path>]');
  process.exit(1);
}

const slug = "practice-test-1";
...
function validateSource(result: Test6ParseResult, enriched: boolean): void {
  ...
  if (questions.some((question) => question.needsReview)) {
    errors.push(`questions need review: ${questions.filter((question) => question.needsReview).map((question) => question.key).join(", ")}`);
  }
  if (result.images.size !== 2 || referencedImages.length !== 2 || new Set(referencedImages).size !== 2) {
    errors.push(`expected 2 uniquely referenced figures, found ${referencedImages.length}/${result.images.size}`);
  }
  if (tableCount !== 7) errors.push(`expected 7 native tables, found ${tableCount}`);
  if (enriched) {
  ...
  if (errors.length) throw new Error(`Practice Test 1 R&W validation failed:\n- ${errors.join("\n- ")}`);
}

function printReport(result: Test6ParseResult): void {
  const questions = result.modules.flatMap((testModule) => testModule.questions);
  console.log("\nPractice Test 1 R&W replacement report");
  for (const testModule of result.modules) {
  ...
  console.log(`Tables: ${questions.filter((question) => [question.passage, question.prompt].some((value) => value?.includes("@@ROW@@"))).length}`);
...
function buildReplacement(
  ...
  if (existing.modules.length !== 3 || existing.questions.length !== 81 || existing.choices.length !== 324) {
    throw new Error("existing Practice Test 1 R&W rows do not match the required 3/81/324 structure");
  }
...
async function main(): Promise<void> {
  ...
  console.log("Imported and verified Practice Test 1 R&W: 3 modules, 81 questions, 324 choices, 2 figures, 7 tables.");
